...
| 11 | 已安装的 `C:\Users\用户名\.codex\skills\deepseek-prism` 副本不会自动同步本次改动 | 安装副本与仓库不一致 | 发布时手动同步（未经用户确认不自动执行） |
...
| 16 | 非 Codex 桌面环境（纯 CLI）可能找不到 sharp | 大图只检测不缩放 | `doctor` 会显示缩放后端状态；可把 `sharp` 安装到技能目录（`node_modules/sharp`）或设置 `VISION_SHARP_PATH` |
