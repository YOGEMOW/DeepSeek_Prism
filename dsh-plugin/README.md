...
- **`prism_see` 工具**：模型传入图片路径/URL 与聚焦问题，返回 VEP/1 紧凑证据（`detail: true` 时输出分节报告）。密钥解析顺序：设置文档 → 环境变量 `SILICONFLOW_API_KEY` / `VISION_API_KEY`。
- **对话发图自动降级**：宿主插件提供可选 `imageFallback` 服务——当前模型不支持图片输入时，harness 的 prompt 准入把用户上传的图片交给本插件转为 VEP/1 文本后入会话，因此文本模型会话可直接上传图片并自动识图（依赖 harness 的 `ImageFallbackService` 接缝，见前置条件）。
...
```powershell
# 在 deepseek-harness checkout 下执行
pnpm dsh plugin --profile web add E:\Git\repositoris\DeepSeek_Prism\dsh-plugin
```
...
### 前置条件：harness 配套改动（一次性，推荐用补丁）

插件依赖 harness 源码的三处配套改动，已打包为 `../harness-patch/dsh-prism-harness.patch`（含设置白名单、发图降级接缝、文本模型图片剥离、前端 VEP 折叠与进度卡片；详情与重建命令见 `../harness-patch/README.md`）：
...
pnpm run test           # node tests/host.spec.mjs（含 mock 视觉 API 端到端）
```

## 已知限制

- 工具不包含本地缓存（技能 CLI 的 SHA-256 缓存仅存在于 `vision.mjs see` 命令路径）。
- bundle 依赖 `deepseek-prism/scripts/vision.mjs` 的相邻目录布局；打包分发（npm pack）需要把该脚本纳入包内后再调整解析逻辑。
