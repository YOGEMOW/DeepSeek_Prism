# @yogemow/deepseek-prism-dsh（已废弃）

> **已废弃（归档）**：本目录为旧「零补丁 B 架构」主线的参考实现，**不再维护、不参与发布**。
> 2026-08 起主线切换为 `packages/plugin-dsh` 的「harness 补丁完整路线」（`prism_see` 工具 + `imageFallback` 接缝 + 设置卡片，依赖 `harness-patch/dsh-prism-harness.patch`，见仓库根 README）。
> 保留此目录用于参考与回滚。

DeepSeek Prism 的 DeepSeek Harness（DSH）插件（零补丁默认的旧主线实现）。
