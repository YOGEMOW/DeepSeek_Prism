...
- **准入证据被 520 字符预算截断**（日常截图代码/日志/OCR 不便）：根因是 `parseVisionResult` 未收到档位预算、始终按 520 默认截断字段（`toVep` 的预算再大也无效）。修复：parse 传入档位 `maxChars`；档位文本预算提高至 2048/4096/8192（输出 token 上限 1024/2048/4096）；编译证据仍含 `[截断]` 时自动升档重取直至完整或最高档；最高档仍截断时追加提示（可用 prism_see --detail 完整提取）；附件路径指针两种部署模式均附带，模型可随时补查。新增"截断自动升档"回归测试；插件 30/30 通过。
