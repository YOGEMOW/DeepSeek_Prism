...
test("toVep 输出 VEP/2 并遵守字符预算（含 g/d/art 分级裁剪）", () => {
  ...
  // 预算紧张时走 compact：仍保留 g/d，最终带 [截断] 不超预算。
  const small = toVep(result, 120);
...
test("callVision 多图：imageDataUrls 一次请求携带全部图片", async () => {
  const { server, port } = await startMockServer((req, res) => {
    let body = "";
    req.on("data", (chunk) => { body += chunk; });
    req.on("end", () => {
      const parsed = JSON.parse(body);
      const images = parsed.messages[0].content.filter((c) => c.type === "image_url");
      assert.equal(images.length, 2, "双图必须在同一请求中");
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({
        choices: [{ message: { content: '{"d":[{"x":0.1,"y":0.1,"w":0.2,"h":0.2,"desc":"差异"}]}' } }],
  ...
  try {
    const provider = {
      id: "siliconflow", name: "test", region: "cn",
      baseUrl: `http://127.0.0.1:${port}/v1`,
      apiKeyEnv: "SILICONFLOW_API_KEY", defaultModel: "glm-test",
      supportsDetail: true, priority: 0, notes: "",
    };
    const meta = await callVision({
      provider,
      apiKey: "sk-test",
      baseUrl: provider.baseUrl,
      model: "glm-test",
      prompt: "对比差异",
      imageDataUrls: ["data:image/png;base64,AQ==", "data:image/png;base64,Ag=="],
      withMeta: true,
    });
    assert.match(meta.text, /\{"d":\[\{"x":0\.1/);
    assert.equal(meta.usage.total_tokens, 12);
  } finally {
    server.close();
  }
});

test("queryBalance：SiliconFlow 解析余额，其他 provider 不查询", async () => {
  const { server, port } = await startMockServer((req, res) => {
  ...
  try {
    ...
    assert.equal(await queryBalance({ baseUrl, apiKey: "sk-test", providerId: "siliconflow" }), 12.34);
    assert.equal(await queryBalance({ baseUrl, apiKey: "sk-test", providerId: "zhipu" }), null);
    assert.equal(await queryBalance({ baseUrl, apiKey: "", providerId: "siliconflow" }), null);
  } finally {
    server.close();
  }
});
