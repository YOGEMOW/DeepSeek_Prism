...
- Cordis 合规审查与优化（2026-08-15）：逐项对照 Cordis/harness 规范审查 `dsh-plugin`——**符合项**：双 half 组合、`name/inject/Config/apply` 命名导出、无默认导出、无模块级可变状态、无全局副作用、fiber 生命周期注册（settings/tools/slots/provide 全部随插件纤维 dispose）、`ctx.get` 可选服务语义、client 的 locale/slot/inject 规范；**优化项**：① 新增 `tests/real-composition.spec.mjs`（2 项）——通过真实 `ctx.plugin()` 装配验证 inject 等待、`imageFallback` 服务注册与 **fiber dispose 清理/幂等**，替代纯手工 stub 的盲区；② 过时文案 VEP/1→VEP/2（模块 JSDoc、工具 detail 描述、注释）；③ `prism_see` execute 未用参数改 `_exec`；Config 双声明保留（harness 惯例）；`pnpm run test` 改为同时运行两个测试文件；插件 13 项测试全部通过，重建 bundle。
## 进行中
